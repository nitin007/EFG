...
describe LenderUserPermissions do
  ...
  context 'recoveries' do
    it { assert user.can_create?(Recovery) }
  end
  ...
  context 'information declaration' do
    it { assert user.can_view?(InformationDeclaration) }
  end
  ...
  context 'Loan Cancel' do
    it { assert user.can_create?(LoanCancel) }
  end

  context 'Loan Demand to Borrower' do
    it { assert user.can_create?(LoanDemandToBorrower) }
  end

  context 'Loan Repay' do
    it { assert user.can_create?(LoanRepay) }
  end

  context 'Loan No Claim' do
    it { assert user.can_create?(LoanNoClaim) }
  end

  context 'Loan Demand Against Government' do
    it { assert user.can_create?(LoanDemandAgainstGovernment) }
  end

  context 'Loan Guarantee' do
    it { assert user.can_create?(LoanGuarantee) }
  end

  context 'loan changes' do
    it { assert user.can_view?(LoanChange) }
    it { assert user.can_create?(LoanChange) }
  end

  context 'loan alerts' do
    it { assert user.can_view?(LoanAlerts) }
  end
